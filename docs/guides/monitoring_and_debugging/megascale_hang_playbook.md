...
3. Run your workload with stack traces enabled. For Cluster Toolkit jobs, add the stack-trace sidecar configuration to the `gcluster job submit` workload definition. Users maintaining legacy XPK jobs should follow the [XPK-specific instructions](https://github.com/AI-Hypercomputer/xpk/blob/main/docs/troubleshooting.md#collect-stack-traces) and use the `--deploy-stacktrace-sidecar` flag.
...
For Cluster Toolkit jobs, configure stack-trace collection in the `gcluster job submit` workload definition. Users maintaining legacy XPK jobs should follow the [XPK-specific instructions](https://github.com/AI-Hypercomputer/xpk/blob/main/docs/troubleshooting.md#collect-stack-traces) and use the `--deploy-stacktrace-sidecar` flag.
